...
@Component({
    selector: 'general-stock',
    templateUrl: './general-stock.component.html',
    styleUrls: ['./general-stock.component.scss'],
  })
  export class GeneralStockComponent implements OnInit {
    ...
    getGeneralStock() {
      ...
      this.inventoryService.getGeneralStockList().subscribe(
        (response: any) => {
    ...
    exportToExcel(): void {
      ...
      writeFile(wb, `AlmacénGeneral.xlsx`);
    }
    ...
    onFileSelected(event: any): void {
      ...
      if (file) {
        ...
        this.inventoryService.uploadGeneralStock(formData).subscribe(
          (response: any) => {
  ...
  deleteMaterial(id: any) {
    this.notification.confirmDialog().then(
      (response) => {
        if (response.isConfirmed) {
          this.inventoryService.deleteGeneralStock(id).subscribe(
            (response: any) => {
