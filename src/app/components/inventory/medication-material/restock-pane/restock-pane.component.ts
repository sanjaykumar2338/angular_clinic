...
@Component({
  selector: 'restock-pane',
  templateUrl: './restock-pane.component.html',
  styleUrls: ['./restock-pane.component.css'],
})
export class RestockMaterialComponent implements OnInit, OnChanges {
  ...
  restockMaterial() {
    ...
    this.inventoryService.reStock(this.restockAside.material.id, {available_stock: newStock}).subscribe(
      (response: any) => {
        this.notificationService.success('Actualizar correctamente');
        this.closeAside();
